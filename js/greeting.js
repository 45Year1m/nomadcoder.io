...
function onLoginSubmit(event) {
    ...
    loginForm.classList.add(HIDDEN_CLASSNAME); // step2. form을 다시 숨겨준다
...
if(savedUsername === null) { // step7. 처음에는 key랑 value가 null이므로
    loginForm.classList.remove(HIDDEN_CLASSNAME); // form에 hidden을 지워주고
    loginForm.addEventListener("submit", onLoginSubmit); // form이 submit될때만 onLoginSubmit함수를 실행 시키도록 한다
} else {
    paintGreetings(savedUsername); // 유저정보가 localstoreage에서 나오고 있다
}